...
class PositionManager:
    ...
    def update(self, current_price: float) -> tuple[float, str | None]:
        """
        Updates stop loss and checks for exit conditions.

        Returns:
            ('STOP_LOSS' | 'TAKE_PROFIT' | None, price_hit | None)
        """
        exit_result = self.check_exit(current_price)
        if exit_result:
            exit_type, exit_price = exit_result
            return exit_type, exit_price  # No need to return SL — we're exiting
...
class BaseStrategy:
    ...
    def _convert_np_to_pd(
        ...
        if isinstance(output, tuple):
            # Convert tuple of numpy arrays to pd.DataFrame and extract first column
...
class NNFXStrategy(BaseStrategy):
    ...
    def generate_trade_plan(
        ...
        signals = self._generate_signals(data_with_indicators, current_position)
