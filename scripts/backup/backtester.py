...
from scripts import strategies, config
...
class PositionCalculator:
    ...
    def calculate_target_pip_value(balance: int, risk_per_trade: float, stop_loss_in_pips):
        """
    ...
    def calculate_current_pip_value(forex_pair: str, units: float, exchange_rate: float,
                                    quote_to_usd_rate: float=None) -> float:
        """
    ...
    def calculate_stop_loss_pips(forex_pair: str, atr: float, atr_multiplier: float=1.5):
        """
    ...
    def calculate_stop_loss_price(direction: str, entry_price: float, atr: float):
        ...
        raw_digits = int(str_price.replace('.', ''))  # Remove the decimal from entry price
        ...
        stop_loss_price = adjusted_str[:decimal_position] + '.' + adjusted_str[decimal_position:]

        return float(stop_loss_price)

    def calculate_trade_units(forex_pair: str, balance: int, risk_per_trade: float, entry_price: float, atr: float, quote_to_usd_rate: float = None):
        """
        Calculates the number of trade units based on the target pip value. (NNFX)
        """
        # Calculate current pip value per lot
        dollars_per_pip_per_lot = PositionCalculator.calculate_current_pip_value(forex_pair, 100_000, entry_price, quote_to_usd_rate)

        # Calculate stop loss in pips
        stop_loss_in_pips = PositionCalculator.calculate_stop_loss_pips(forex_pair, atr)

        # Calculate target pip value
        target_pip_value = PositionCalculator.calculate_target_pip_value(balance, risk_per_trade, stop_loss_in_pips)
    ...
    def calculate_required_margin(forex_pair: str, units: int, leverage: int, price: float, quote_to_usd_rate: float = None) -> float:
        """
        ...
        if quote_to_usd_rate is None:
            raise ValueError(f"quote_to_usd_rate is required for non-USD quote currency ({quote_currency})")

        margin_in_usd = margin_in_quote * quote_to_usd_rate
        return round(margin_in_usd, 2)

    
    def calculate_profit_from_pips(forex_pair: str, pip_change: float, units: float, exit_price: float, quote_to_usd_rate: float = None) -> float:
        """
        ...
        Returns:
            float: Profit in USD
        """
        pip_value = PositionCalculator.calculate_current_pip_value(forex_pair, units, exit_price, quote_to_usd_rate)

        return round(pip_change * pip_value, 2)

    def calculate_pip_change(forex_pair: str, entry_price: float, exit_price: float, direction: str) -> float:
        """
...
class Trade:
    ...
    """
    def __init__(self, forex_pair: str, entry_timestamp, trade_type: str, entry_price: float, units: int, balance: float, leverage: int, margin_required: float, pip_value: float):
    ...
    def close_trade(self, exit_timestamp, exit_price: float, 
                    commission_per_lot: float, quote_to_usd_exit_rate: float=None):
        if self.exit_price is not None:
            return  # Prevent closing a trade twice

        self.exit_price = exit_price

        self.net_pips = PositionCalculator.calculate_pip_change(self.forex_pair, self.entry_price,
                                                             self.exit_price, self.trade_type)

        # Calculate PnL based on trade direction
        self.pnl = PositionCalculator.calculate_profit_from_pips(self.forex_pair, self.net_pips, self.units, 
                                                                 self.exit_price, quote_to_usd_exit_rate)
        ...
        self.duration = int((exit_time - entry_time).total_seconds() / 60)  # duration in minutes
        self.return_pct = round((self.pnl / self.balance_before_trade) * 100, 2) if self.balance_before_trade != 0 else None

    def __repr__(self):
        return (f"{self.entry_timestamp}: {self.trade_type} @ {self.entry_price}, "
...
class Backtester:
    def __init__(self, strategy: strategies.BaseStrategy, forex_pair: str, timeframe: str, initial_balance=10_000, risk_per_trade=0.02,
                 commission_per_lot=6, slippage=0.0002, leverage=50):
        """
        ...
        self.base_currency, self.quote_currency = self.forex_pair[:3], self.forex_pair[3:]
        ...
        self.backtest_sqlhelper = BacktestSQLHelper(f"{config.BACKTESTING_FOLDER}/{config.BACKTESTING_DB_NAME}")
        self.forex_pair_id = self.backtest_sqlhelper.get_forex_pair_id(f"{self.base_currency}/{self.quote_currency}")
        self.strategy_config_id = self.backtest_sqlhelper.upsert_strategy_configuration(self.strategy.NAME, self.strategy.DESCRIPTION, self.strategy.PARAMETER_SETTINGS)

        # Get historical data
        data_sqlhelper = HistoricalDataSQLHelper(f"{config.DATA_FOLDER}/{forex_pair}.db")
        self.data = data_sqlhelper.get_historical_data(table=timeframe, columns=self.strategy.REQUIRED_COLUMNS)
        self.data_start_date, self.data_end_date = self.data['Timestamp'].min(), self.data['Timestamp'].max()
        ...
        if self.quote_currency != "USD":
            secondary_data_sqlhelper = HistoricalDataSQLHelper(f"{config.DATA_FOLDER}/{self.quote_currency}USD.db")
            self.quote_to_usd_data = secondary_data_sqlhelper.get_historical_data(table=timeframe)
    ...
    def run_backtest(self):
        ...
        for index, row in self.data.iterrows():
            ...
            if self.position == 1 and exit_long and self.entry_trade:
                ...
                self.entry_trade.close_trade(timestamp, exit_price, self.commission_per_lot, quote_to_usd_rate)
            ...
            if self.position == 0 and entry_long:
                ...
                trade_units = PositionCalculator.calculate_trade_units(self.forex_pair, self.balance, self.risk_per_trade, entry_price, atr, quote_to_usd_rate)
                margin_required = PositionCalculator.calculate_required_margin(self.forex_pair, trade_units, self.leverage, entry_price, quote_to_usd_rate)
                if margin_required > self.margin_remaining:
                    continue

                pip_value = PositionCalculator.calculate_current_pip_value(self.forex_pair, trade_units, entry_price, quote_to_usd_rate)
                
                self.entry_trade = Trade(self.forex_pair, timestamp, 'BUY', entry_price, trade_units, self.balance, self.leverage, margin_required, pip_value)
            ...
            # Enter Short Position
                ...
                self.entry_trade = Trade(self.forex_pair, timestamp, 'SELL', entry_price, trade_units, self.balance, self.leverage, margin_required, pip_value)
                self.trades.append(self.entry_trade)
                self.position = -1
    ...
    def save_run(self) -> int:
        ...
        if total_trades == 0:
            ...
            self.run_id = self.backtest_sqlhelper.insert_backtest_run(self.forex_pair_id, self.strategy_config_id, metrics_df)
            return self.run_id

        # Basic aggregated metrics.
        winning_trades = sum(1 for trade in self.trades if trade.pnl is not None and trade.pnl > 0)
        gross_profit = sum(trade.pnl for trade in self.trades if trade.pnl is not None and trade.pnl > 0)
        gross_loss = sum(trade.pnl for trade in self.trades if trade.pnl is not None and trade.pnl < 0)
        initial_balance = self.initial_balance
        ...
        max_drawdown = getattr(self, "max_drawdown", 0.0)
        average_trade_duration = sum(trade.duration for trade in self.trades if trade.duration is not None) / total_trades

        # Compute trade returns (as percentages) for Sharpe, Sortino, and Trade Expectancy.
        trade_returns = [trade.return_pct for trade in self.trades if trade.return_pct is not None]
        ...
        sortino_ratio = (mean_return / std_downside) if std_downside != 0 else float('inf')
        ...
        wins_pct = [trade.return_pct for trade in self.trades if trade.return_pct is not None and trade.return_pct > 0]
        losses_pct = [trade.return_pct for trade in self.trades if trade.return_pct is not None and trade.return_pct < 0]
        avg_win_pct = np.mean(wins_pct) if wins_pct else 0.0
        avg_loss_pct = np.mean(losses_pct) if losses_pct else 0.0
        win_loss_ratio = (avg_win_pct / abs(avg_loss_pct)) if avg_loss_pct != 0 else float('inf')
        ...
        trade_expectancy_pct = (win_rate * avg_win_pct) - ((1 - win_rate) * abs(avg_loss_pct))
        ...
        max_pct_margin_required = max(
            (trade.margin_required / trade.balance_before_trade * 100 for trade in self.trades if trade.balance_before_trade > 0),
            default=0.0
        )
        ...
        metrics["Profit_Factor"] = round(profit_factor, 2) if profit_factor != float('inf') else profit_factor
        ...
        metrics["Sharpe_Ratio"] = round(sharpe_ratio, 2) if not np.isnan(sharpe_ratio) else sharpe_ratio
        metrics["Sortino_Ratio"] = round(sortino_ratio, 2) if sortino_ratio != float('inf') else sortino_ratio
        metrics["Calmar_Ratio"] = round(calmar_ratio, 2) if calmar_ratio != float('inf') else calmar_ratio
        metrics["Recovery_Factor"] = round(recovery_factor, 2) if recovery_factor != float('inf') else recovery_factor
        metrics["Win_Loss_Ratio"] = round(win_loss_ratio, 2) if win_loss_ratio != float('inf') else win_loss_ratio
    ...
    def save_trades(self):
        ...
        trade_dict = {
            ...
            "Direction": [1 if trade.trade_type == 'BUY' else -1 for trade in self.trades],
            ...
            "Margin_Required_Pct": [round(trade.margin_required / trade.balance_before_trade, 2) for trade in self.trades],
