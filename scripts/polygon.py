...
logging.basicConfig(level=20, datefmt='%m/%d/%Y %H:%M:%S', format='[%(asctime)s] %(message)s')
...
class Polygon:
    ...
    def fetch_historical_data(self, ticker: str, multiplier: int, timespan: str, from_date: str, to_date: str) -> pd.DataFrame:
        """
        ...
        def get_aggs():
            for agg in self.client.list_aggs(ticker, multiplier, timespan, from_=from_date, to=to_date, limit=50000):
        ...
        data = data[['timestamp', 'open', 'high', 'low', 'close', 'volume', 'vwap', 'transactions']]
    ...
    def filter_collected_data_dates(self, existing_data: pd.DataFrame, from_date: str, to_date: str) -> dict:
        """
        ...
        day_before_to_date = (pd.to_datetime(to_date) - pd.Timedelta(days=1)).strftime('%Y-%m-%d')
        ...
        if filter_from:
            ...
            most_recent_date = existing_data[existing_data['Timestamp'] > from_date]['Timestamp'].max()
        ...
        if pd.to_datetime(filtered_dates['from_date']) == pd.to_datetime(to_date) - pd.Timedelta(days=1):
            return None

        # Check if to_date is in any timestamp, if so, get the furthest back date before that point
        if filter_to:
            # Get most recent date past initial from date
            least_recent_date = existing_data[existing_data['Timestamp'] < to_date]['Timestamp'].min()
    ...
    def collect_data(self, pair: str) -> None:
        ...
        for timeframe, days_back in timeframe_data_collection.items():
            ...
            # Filter data collection range based on the initial from date
            existing_data = sql_helper.get_historical_data(db_table, from_date=from_date, to_date=to_date)

            if existing_data is not None:
                filtered_dates = self.filter_collected_data_dates(existing_data, from_date, to_date)
                ...
                from_date, to_date = filtered_dates['from_date'], filtered_dates['to_date']
            
            # Fetch data from Polygon API
            collected_data = self.fetch_historical_data(ticker, multiplier, timespan, from_date, to_date)
